...
p.s: перегрузка - определение методов с одинаковой сигнатурой (тип возвращаемого значения + имя метода), но с разными аругментами


Рассуждения по задаче:
для Reader дата рождения хранится в отдельном классе, номер телефона это переменная целочисленная.
книга отдельный класс